...
type AuthGuardOutput<T> = T extends InputReturnUserTrue
  ? User
...
export const authGuard = async <T extends AuthGuardInput>(
  input: T
): Promise<AuthGuardOutput<T>> => {
  ...
  // return if no user is requested
  if (!returnUser) {
    return undefined as AuthGuardOutput<T>;
  }
  ...
  return user as AuthGuardOutput<T>;
};
